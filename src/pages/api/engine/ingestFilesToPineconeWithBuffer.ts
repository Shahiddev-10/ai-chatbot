...
export default async function handler(req: any, res: any) {
  try {
    ...
    const myLoader = new MyBufferLoader(req.body);
    ...
    const rawDocs = await myLoader.parse(req.body);
    console.log(
      '🚀 ~ file: ingestBuffer.js:76 ~ handler ~ rawDocs:',
    ...
    res.status(201).json({ data: rawDocs, name: 'John Doe' });
