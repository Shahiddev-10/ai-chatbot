...
import useIsSubscriptionActive, {
  ACTIVE_STATUSES,
} from './hooks/use-is-subscription-active';
...
function isAdmin(role: MembershipRole | undefined) {
  return role === MembershipRole.Admin;
}

export function useActiveSubsciption() {
  const isActiceSubscription = useIsSubscriptionActive()
  if (isActiceSubscription) {
    return isActiceSubscription;
  }
  if (configuration.isFreePlanSupported) {
    return true
  }
  return isActiceSubscription;
}
export function isSubscriptionActiveContext(ctx: any) {
  const status = ctx?.organization?.subscription?.status;
  if (!status) {
    return false;
  }
...
export function useCreateNewThing(userRole: MembershipRole | undefined) {
  const isPayingUser = useIsSubscriptionActive();

  return isPayingUser && isAdmin(userRole);
}

// isfreeplaned allowed or not
